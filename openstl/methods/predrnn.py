...
class PredRNN(Base_method):
    ...
    def _predict(self, batch_x, batch_y, **kwargs):
        ...
        # preprocess
        test_ims = (
            torch.cat([batch_x, batch_y], dim=1).permute(0, 1, 3, 4, 2).contiguous()
        )
        ...
        real_input_flag = torch.zeros(
            (
                ...
                self.args.patch_size**2 * img_channel,
            )
        ).to(self.device)
        ...
        pred_y = (
            img_gen[:, -self.args.aft_seq_length :].permute(0, 1, 4, 2, 3).contiguous()
        )
