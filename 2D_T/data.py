"""
Dataset for 2D + T microstructure data
"""
...
class MicroS_Dataset(Dataset):
    ...
    def _parse_config_file(self):
        self.video_list = [
            Video(x.strip().split(), self.root_path) for x in open(self.config_path)
        ]
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(self.n_frames_total):
            file_exists = exists(
                os.path.join(sample.path, self.imagefile_template.format(frame_idx))
            )
            if not file_exists:
                print(
                    "file do not exist!",
                    os.path.join(
                        sample.path, self.imagefile_template.format(frame_idx)
                    ),
                )
            else:
                image = Image.open(
                ...
                if image.size[0] == 1520:
                    ...
                    print(
                        "Size anomaly, file should be skipped:",
                        self.imagefile_template.format(frame_idx),
                    )
        ...
        # rearrange for video transformers
        input_frames = rearrange(input_frames, "d c h w -> c d h w")
        output_frames = rearrange(output_frames, "d c h w -> c d h w")
...
def main():
    ...
    videos_root = "/home/monicar/swin_transformers/2d_imgs"
    train_path = os.path.join(videos_root, "config_file")
