...
class Potts_Param:
    ...
    def _compute_range(self, param_dict: dict) -> List[float]:
        ...
        # Construct the range list based on start, stop, and step
        #value_range = list(range(int(details['start'] * 10), int(details['stop'] * 10 + details['step']), int(details['step'] * 10)))
        # Adjust the range to float if needed (as range only supports integers)
        #value_range = [x / 10 for x in value_range]
...
if __name__ == "__main__":
    ...
    filename = os.path.join(dirname, './small_params.yaml')
