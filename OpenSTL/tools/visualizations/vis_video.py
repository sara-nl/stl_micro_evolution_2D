...
def parse_args():
    parser = argparse.ArgumentParser(description="Visualization of a STL model")
    ...
    parser.add_argument(
        "--work_dirs",
        "-w",
        default="/home/monicar/predictive_zoo/OpenSTL/work_dirs/pf_predrnn",
        type=str,
...
def main():
    ...
    for method in method_list:
        ...
        if "weather" in args.dataname:
            ...
            inputs = show_weather_bench(
                inputs[idx, 0:ncols, ...], src_img=None, cmap="GnBu"
            ).transpose(0, 3, 1, 2)
            trues = show_weather_bench(
                trues[idx, 0:ncols, ...], src_img=None, cmap="GnBu"
            ).transpose(0, 3, 1, 2)
        elif "taxibj" in args.dataname:
            inputs = show_taxibj(inputs[idx, 0:ncols, ...], cmap="viridis").transpose(
                0, 3, 1, 2
            )
            trues = show_taxibj(trues[idx, 0:ncols, ...], cmap="viridis").transpose(
                0, 3, 1, 2
            )
        else:
    ...
    for i, method in enumerate(method_list):
        ...
        if "weather" in args.dataname:
            preds = show_weather_bench(
                predicts_dict[method][idx, 0:ncols, ...],
                src_img=None,
                cmap="GnBu",
        ...
        if i == 0:
            show_video_line(
                ...
                vmax=0.6,
                cbar=False,
                out_path="{}/{}_input{}".format(
                    args.save_dirs, args.dataname + c_surfix, str(idx) + ".png"
                ),
                format="png",
                use_rgb=use_rgb,
            )
            ...
            #    inputs.copy(),
            #    use_rgb=use_rgb,
            #    out_path="{}/{}_{}_{}_input".format(
            #        args.save_dirs, args.dataname + c_surfix, method, idx
            #    ),
            # )
            # show_video_gif_single(
            #    trues.copy(),
            #    use_rgb=use_rgb,
            #    out_path="{}/{}_{}_{}_true".format(
            #        args.save_dirs, args.dataname + c_surfix, method, idx
            #    ),
            # )
        ...
        show_video_line(
            ...
            cbar=False,
            out_path="{}/{}_{}_{}".format(
                args.save_dirs, args.dataname + c_surfix, method, str(idx) + ".png"
            ),
            format="png",
            use_rgb=use_rgb,
        )
