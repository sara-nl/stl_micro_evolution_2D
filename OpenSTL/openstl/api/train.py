...
class BaseExperiment(object):
    ...
    def _get_data(self, dataloaders=None):
        ...
        if dataloaders is None:
            print("pre seq length should be 8", self.config)
            self.train_loader, self.vali_loader, self.test_loader = get_dataset(
                self.args.dataname, self.config
            )
        else:
    ...
    def _save(self, name=""):
        ...
        checkpoint = {
            ...
            if not self._dist
            else weights_to_cpu(self.method.model.module.state_dict()),
    ...
    def _load(self, name=""):
        ...
        filename = (
            name if osp.isfile(name) else osp.join(self.checkpoints_path, name + ".pth")
        )
        try:
    ...
    def display_method_info(self):
        ...
        if self.args.method in ["simvp", "tau"]:
            input_dummy = torch.ones(1, self.args.pre_seq_length, C, H, W).to(
                self.device
            )
            ...
            _tmp_input1 = torch.ones(1, self.args.pre_seq_length, C, H, W).to(
                self.device
            )
            _tmp_input2 = torch.ones(1, self.args.aft_seq_length, C, H, W).to(
                self.device
            )
            ...
            _tmp_input = torch.ones(1, self.args.total_length, Hp, Wp, Cp).to(
                self.device
            )
            _tmp_flag = torch.ones(1, self.args.aft_seq_length - 1, Hp, Wp, Cp).to(
                self.device
            )
            ...
            _tmp_flag = torch.ones(1, self.args.total_length - 2, Hp, Wp, Cp).to(
                self.device
            )
            input_dummy = (_tmp_input, _tmp_flag)
    ...
    def train(self):
        ...
        recorder = Recorder(
            verbose=True, early_stop_time=min(self._max_epochs // 10, 10)
        )
        ...
        for epoch in range(self._epoch, self._max_epochs):
            ...
            num_updates, loss_mean, eta = self.method.train_one_epoch(
                self, self.train_loader, epoch, num_updates, eta
            )
            ...
            if epoch % self.args.log_step == 0:
                ...
                with torch.no_grad():
                    vali_loss, mse_loss, mae_loss = self.vali()
                ...
                if self._rank == 0:
                    ...
                    self.writer.add_scalar("Train/Loss", loss_mean.avg, epoch + 1)
                    self.writer.add_scalar("Validation/Loss", vali_loss, epoch + 1)
    ...
    def test(self):
        ...
        if "weather" in self.args.dataname:
            ...
            channel_names = (
                self.test_loader.dataset.data_name
                if "mv" in self.args.dataname
                else None
            )
        else:
