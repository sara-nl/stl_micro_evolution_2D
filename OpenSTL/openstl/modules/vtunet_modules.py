...
def _get_num_patches(img_size, patch_size):
    """
    Args:
        img_size (tuple): The dimensions of the image (could be 2D or 3D).
        patch_size (tuple): The size of each patch (should match the dimensionality of img_size).
    Returns:
        int: The total number of patches.
    """
    patches_resolution = [i_dim // p_dim for i_dim, p_dim in zip(img_size, patch_size)]
    num_patches = 1
    for dim in patches_resolution:
        num_patches *= dim
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix, prev_v, prev_k, prev_q, is_decoder):
        ...
        if any(i > 0 for i in shift_size):
            ...
            attn_mask = None
        # partition windows
        x_windows = _window_partition(shifted_x, window_size)  # B*nW, Wd*Wh*Ww, C
        # W-MSA/SW-MSA
...
# TODO PATCH EXPAND AND PATCH EXPAND UP - careful about when there are used
...
class FinalPatchExpand_X4(nn.Module):
    def __init__(self, dim, dim_scale_time=2, dim_scale=4, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.dim_scale_time = dim_scale_time
        self.dim_scale = dim_scale
        self.expand = nn.Linear(
            dim, dim_scale_time * dim_scale * dim_scale * dim, bias=False
        )  # 4 * 16 * dim? when patch size is 4,4,4
        self.output_dim = dim
    ...
    def forward(self, x):
        ...
        x = rearrange(
            ...
            p1=self.dim_scale_time,
            p2=self.dim_scale,
            p3=self.dim_scale,
            c=C
            // (
                self.dim_scale_time * self.dim_scale**2
            ),  # TODO look at here, the final layer
        )
        x = self.norm(x)
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(
        ...
        depth,
        num_heads,
        window_size=(7, 7, 7),  # TODO CHECK window size
        mlp_ratio=4.0,
        qkv_bias=True,
        qk_scale=None,
        drop=0.0,
    ...
    for d in (
        ...
        for h in (
            ...
            for w in (
                ...
                img_mask[:, d, h, w, :] = cnt
                cnt += 1
    mask_windows = _window_partition(img_mask, window_size)  # nW, ws[0]*ws[1]*ws[2], 1
...
class SwinTransformer3D_Sys(nn.Module):
    ...
    def __init__(
        ...
        self.norm = norm_layer(self.num_features)  # TODO this or features up?
        self.norm_up = norm_layer(self.embed_dim)

        if self.final_upsample == "expand_first":
            print("---final upsample expand_first---")
            self.up = FinalPatchExpand_X4(
                dim_scale_time=patch_size[0],
                dim_scale=patch_size[1],
                dim=embed_dim,
            )
            # classification
    ...
    def forward_features(self, x):
        ...
        # format B x 8*C x D/2 x H/32 x W/32
        x = rearrange(x, "n c d h w -> n d h w c")
        x = self.norm(x)
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformer3D_Sys, self).train(mode)
        self._freeze_stages()
